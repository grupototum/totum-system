...
import { Plus, Pencil, Copy, Trash2, X, Loader2, FolderOpen, ChevronDown, ChevronRight, LayoutGrid, List } from "lucide-react";
...
export function ProjectTemplateManager() {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      reportError("Erro ao carregar templates:", err, "project_templates_load");
  ...
  const duplicate = async (t: ProjectTemplate) => {
    try {
      ...
      reportError("Erro ao duplicar template:", err, "project_template_duplicate");
      const message = err instanceof Error ? err.message : String(err);
  ...
  const deleteTemplate = async (id: string) => {
    try {
      ...
      reportError("Erro ao remover template:", err, "project_template_delete");
      const message = err instanceof Error ? err.message : String(err);

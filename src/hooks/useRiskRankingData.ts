...
export function useRiskRankingData() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: checklists }, { data: items }] = await Promise.all([
        supabase.from("delivery_checklists").select("client_id, fulfillment_pct, clients(name)"),
        supabase.from("delivery_checklist_items").select("checklist_id, status, delivery_checklists(client_id)"),
      ]);

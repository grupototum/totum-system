...
const STEPS = [
  { title: "Informações da Empresa", subtitle: "Dados principais do cliente", icon: Building2 },
  { title: "Sobre o Negócio", subtitle: "Contexto do cliente", icon: Briefcase },
  { title: "Público-Alvo", subtitle: "Quem são os clientes ideais?", icon: Target },
  { title: "Identidade Visual", subtitle: "Aparência da marca", icon: Palette },
  { title: "Operação e Contatos", subtitle: "Detalhes operacionais", icon: Settings2 },
];
...
export default function EditClient() {
  ...
  const { client: rawClient, loading, updateClient } = useClientById(clientId);
  const originalFormRef = useRef<FormData | null>(null);

  useEffect(() => {
    if (!rawClient) return;
    const data = rawClient as any;
                ...
                <div className="flex justify-between items-center mt-8 pt-4 border-t border-border/30">
                  ...
                  {step < 4 ? (
                    ...
                    <Button onClick={submit} disabled={saving} className="bg-emerald-600 hover:bg-emerald-700 text-white text-sm">
                      {saving ? <><Loader2 className="w-4 h-4 mr-1 animate-spin" /> Salvando...</> : <><Check className="w-4 h-4 mr-1" /> Salvar Alterações</>}
                    </Button>
                  )}
                </div>
            ...
            <div className="sticky top-24">
              <Card className="border-border/40 bg-card/80">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="text-center">
                    <Badge variant="outline" className="text-[10px] bg-sky-500/10 text-sky-400 border-sky-500/30">Editando...</Badge>
                  </div>
